...
def resource_path(relative_path): #was necessary for creating the exe
...
class MatplotlibWidget(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        loadUi(resource_path("MS_UI_File.ui"), self)
        ...
        self.lineEdit_19.returnPressed.connect(self.error_handling_range_vis)
        self.lineEdit_20.setText("10:100:10")
        self.lineEdit_20.returnPressed.connect(self.error_handling_range_nir)
    ...
    def error_handling_range_vis(self):
    ...
    def error_handling_range_nir(self):
        acquisition_range = self.lineEdit_20
        start_time, end_time, step_size = map(int, acquisition_range.split(":"))
    ...
    def NIR (self): 
        ...
        Channel_NIR = int(self.spinBox_4.text())
        Current_NIR = float(self.lineEdit_7.text())
        acquisition_range = self.lineEdit_20.text()
        start_time, end_time, step_size = map(int, acquisition_range.split(":"))
        ...
        if not self.checkBox_13.isChecked():  # No range of acquisition times
        ...
        # Loop through the range of acquisition times
            for acquisition_time in range(start_time, end_time + 1, step_size):
                ...
                spec1 = ts.spec_measure(acquisition_time, NumberofScans_NIR)  # Dark measurement
                spec1 = np.array(spec1)
                self.Dark_NIR = spec1
                # switch on light measure data
                spec2 = ts.measure(acquisition_time, NumberofScans_NIR, Channel_NIR,
                                   Current_NIR)  # tint integrationszeit ms , averages, channel, power
                spec2 = np.array(spec2)
                self.Raw_NIR = spec2
                self.sample2 = spec2 - spec1
    ...
    def VISNIR(self):
        vis_acquisition_range = self.lineEdit_19.text()
        vis_start_time, vis_end_time, vis_step_size = map(int, vis_acquisition_range.split(":"))
        nir_acquisition_range = self.lineEdit_20.text()
        nir_start_time, nir_end_time, nir_step_size = map(int, nir_acquisition_range.split(":"))
        ...
        if not self.checkBox_12.isChecked() and not self.checkBox_13.isChecked():  # No range of VIS or NIR acquisition times
            ts.set_msm('VIS')
            #Warm-up time for lamp
            command = '*PARA:SDEL '+ str(self.lineEdit_6.text())
            ts.send_command("'"+ command +"'")
        ...
        if self.checkBox_12.isChecked():
            # Loop through the range of acquisition times
            for vis_acquisition_time in range(vis_start_time, vis_end_time + 1, vis_step_size):
                ts.set_msm('VIS')
                # Warm-up time for lamp
                command = '*PARA:SDEL ' + str(self.lineEdit_6.text())
                ts.send_command("'" + command + "'")

                spec1 = ts.spec_measure(vis_acquisition_time, NumberofScans_VIS)  # Dark measurement
                spec1 = np.array(spec1)
                self.Dark_VIS = spec1
                # switch on light measure data
                spec2 = ts.measure(vis_acquisition_time, NumberofScans_VIS, Channel_VIS,
                                   Current_VIS)  # tint integrationszeit ms , averages, channel, power
                spec2 = np.array(spec2)
                self.Raw_VIS = spec2
                self.sample1 = spec2 - spec1
        ...
        if self.checkBox_13.isChecked():
            # Loop through the range of acquisition times
            for nir_acquisition_time in range(nir_start_time, nir_end_time + 1, nir_step_size):
                # Warm-up time for lamp
                ts.set_msm('NIR')
                command = '*PARA:SDEL ' + str(self.lineEdit_8.text())
                ts.send_command("'" + command + "'")

                spec1 = ts.spec_measure(nir_acquisition_time, NumberofScans_NIR)  # Dark measurement
                spec1 = np.array(spec1)
                self.Dark_NIR = spec1
                # switch on light measure data
                spec2 = ts.measure(nir_acquisition_time, NumberofScans_NIR, Channel_NIR,
                                   Current_NIR)  # tint integrationszeit ms , averages, channel, power
                spec2 = np.array(spec2)
                self.Raw_NIR = spec2
                self.sample2 = spec2 - spec1
    ...
    def get_plot_cycleAll(self):
        ...
        #Visible
        self.MplWidget_1.canvas.axes.plot(self.wavelengths1, self.raw_vis)
        ...
        #NIR
        self.MplWidget_2.canvas.axes.plot(self.wavelengths2, self.raw_nir)
        self.MplWidget_2.canvas.axes.legend(labels=legend_labels, loc='upper right')
        self.MplWidget_2.canvas.axes.set_xlim(950, 1800)  # Set x-axis range
        ...
        # FL 365
        self.MplWidget_3.canvas.axes.plot(self.wavelengths1, self.raw_FL365)
        ...
        # FL 405
        self.MplWidget_4.canvas.axes.plot(self.wavelengths1, self.raw_FL405)
